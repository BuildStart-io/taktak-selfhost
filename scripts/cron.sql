...
declare
  ...
  jobs text[][] := array[
    ...
    ['taktak-payment-followups',  '*/10 * * * *', 'send-payment-followups'],
    ['taktak-referral-broadcast', '*/12 * * * *', 'send-referral-broadcast'],
    ['taktak-notify-buyers',      '*/5 * * * *',  'notify-buyers'],
    ['taktak-onepay-reconcile',   '*/5 * * * *',  'reconcile-onepay-pending'],
    ['taktak-wasender-health',    '*/10 * * * *', 'wasender-health'],
    ['taktak-waha-health',        '*/10 * * * *', 'waha-health'],
    ['taktak-revenue-report',     '30 2 * * *',   'send-revenue-report'],
    ['taktak-notion-analytics',   '0 3 * * *',    'push-notion-analytics']
  ];
  j text[];
begin
